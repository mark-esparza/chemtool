...
async function getGeminiPubChemFallback(q: string): Promise<CompoundRecord> {
  try {
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            descriptionSource: { type: Type.STRING, description: "Source label for the data origin" },
            synonyms: {
              type: Type.ARRAY,
    ...
    return {
      ...
      // These values are generated by the language model, not retrieved from
      // PubChem. Label them honestly and flag them so the UI can warn the user.
      descriptionSource: "AI-generated estimate (unverified)",
      is_estimated: true,
...
function getLocalChemicalFallback(q: string): CompoundRecord {
  ...
  const database: Record<string, {
    ...
    naproxen: {
      ...
      descriptionSource: "Built-in reference dataset",
  ...
  const matchedKey = Object.keys(database).find(k => norm === k || norm.includes(k));
  
  if (matchedKey) {
    const data = database[matchedKey];
    return {
      ...data,
      // Curated, real literature values for a known compound.
      is_estimated: false,
  ...
  return {
    ...
    hba: 3,
    rotatable_bonds: 3,
    description: `Placeholder record generated locally for "${q}" — this compound was not found in PubChem or the built-in dataset. The structure and properties below are synthetic placeholders, not real data.`,
    descriptionSource: "Generated placeholder (not real data)",
    is_estimated: true,
...
async function fetchPubChemData(q: string): Promise<CompoundRecord | null> {
  ...
  const matchedKey = knownKeys.find(k => norm === k || norm.includes(k));
  if (matchedKey) {
  ...
  try {
    ...
    return {
      ...
      description,
      descriptionSource,
      // Real data retrieved live from the PubChem PUG REST API.
      is_estimated: false,
      descriptionUrl,
      synonyms,
      reportUrl: `https://pubchem.ncbi.nlm.nih.gov/compound/${cid}`,
